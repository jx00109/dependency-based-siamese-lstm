...
flags.DEFINE_float('alpha_weight', 0.7, 'weight of sentence subj and obj') # 此处需要修改
flags.DEFINE_integer('pre_num_epoch', 1501, 'num epoch')
flags.DEFINE_integer('num_epoch', 17601, 'num epoch')
flags.DEFINE_string('out_dir', os.path.abspath(os.path.join(os.path.curdir, "results/alpha-0.7")),
                    'output directory') #此处需要修改
flags.DEFINE_string('pre_logs_dir', './logs/pre-model-0.7.txt', 'pre train logs') #此处需要修改
flags.DEFINE_string('logs_dir', './logs/model-0.7.txt', 'train logs') #此处需要修改
...
class Config(object):
    ...
    pre_checkpoint_every = FLAGS.pre_check_point_every
